 ...
 */
public class OrangeCargoPipeline implements Pipeline {
    ...
    @Override
    public void update() {
        Core.inRange(mCaptureCopy, ColorConstants.kOrangeLowerBoundHSV, ColorConstants.kOrangeUpperBoundHSV, mCaptureCopy); // masks image to only allow orange objects
        ...
        if (mContoursCandidates.size() != 0) {
            ...
            Imgproc.drawContours(Stream.getCameraCaptures()[indexOfCameraCapture], mContoursCandidates, mContoursCandidates.indexOf(largestContour), ColorConstants.kOrangeLowerBoundHSV, 5);
        }
